//////////////////////////////////////////////////////////////////////////////
// \file aes.c
// \brief From: https://github.com/kokke/tiny-AES-c Slightly modified to better suit
//		Vivado-HLS for Synthesis of Digital Systems Laboratory.
//////////////////////////////////////////////////////////////////////////////
...
void AES_init_ctx_iv(struct AES_ctx *ctx, const uint8_t *key,
                     const uint8_t *iv) {
  KeyExpansion(ctx->RoundKey, key);
  for(int k = 0; k<AES_BLOCKLEN; ++k)ctx->Iv[k] = iv[k];
...
void AES_CTR_xcrypt_buffer(struct AES_ctx *ctx, uint8_t *buf, uint32_t length) {
  ...
  unsigned i;
  int bi;
  AES_CTR_xcrypt_buffer_label0:for (i = 0, bi = AES_BLOCKLEN; i < length; ++i, ++bi) {
    if (bi == AES_BLOCKLEN) /* we need to regen xor compliment in buffer */
    {
      AES_CTR_xcrypt_buffer_label1:for(int k = 0; k<AES_BLOCKLEN; ++k)buffer[k] = ctx->Iv[k];
...
//      Cipher((state_t *)buffer, ctx->RoundKey);
      ...
      AES_CTR_xcrypt_buffer_label2:for (bi = (AES_BLOCKLEN - 1); bi >= 0; --bi) {
        /* inc will overflow */
        if (ctx->Iv[bi] == 255) {
          ctx->Iv[bi] = 0;
          continue;
